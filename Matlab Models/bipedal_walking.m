...
links(3) = Link([    0       0    L3   -pi/2   ], 'standard');

%a single leg
leg = SerialLink(links, 'name', 'leg', 'offset', [0   0.707  -1.514]);

% define the rectangular path taken by the foot
path = [35 0 -5; 31 0 0; 31 0 1; 35 0 5; 35 0 -5 ]*0.01;
  
timeseg = [0.5 1.5 1.5 1.5 5.0]';

y = mstraj(path, [], timeseg, path(1,:), 0.1, 0);

qcycle = leg.ikine( transl(y), [], [1 1 1 0 0 0] );
...
legs(1) = SerialLink(leg, 'name', 'leg1', 'base', transl(0, 0, 0)*troty(pi/2));
legs(2) = SerialLink(leg, 'name', 'leg2', 'base', transl(0, -W, 0)*troty(pi/2));
...
while 1
    ...
    legs(2).plot( gait(qcycle, k, 50, 1), plotopt);
    drawnow
    k = k+1;
end
